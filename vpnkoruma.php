<?php
/**
  twitter.com/bugresearcher
  www.bugresearcher.com
**/

//////////////// AYARLAR ////////////////
/*	Server IP				*/	$teamspeak['address'] = '';
/*	UDP Portu				*/	$teamspeak['udp'] = '9987';
/*	Query Port				*/	$teamspeak['tcp'] = '10011';
/*	Query Kullanıcı				*/	$teamspeak['login'] = 'serveradmin';
/*	Query Şifresi				*/	$teamspeak['password'] = '';
/*	Bot İsmi			*/	$bot['name'] = 'VPN Guard';
/*	Bot Varsayılan Kanal		*/	$bot['default_channel'] = 1; 
/*	Interval Kanal			*/	$bot['interval'] = 1; 
/*	Poke Atma			*/	$poke = "";
/*	Atma Mesajı			*/	$atmayazisi = 'VPN YASAK';
/*	VPN Grub İdisi	*/	$vpn_izin_verilengrup = array(231);
/*	Sunucu İD				*/	$default = 1;
/*	Yedekleme Saniyesi			*/	$log = 1;
    
//////////////// Ayarlar Bitti //////////////// 

//////////////// FONKSIYONLAR ////////////////
date_default_timezone_set('Europe/İstanbul');


//VPN Test Durumu
function vpn($kullanicip)
{
	/*     ayar     */
	$portac = 0;
	$portat = 1; 
	$portyakala = '';
	/*    ayar bitti     */
	
	/* ############### TCP Başladı ############### */
	$portumuz = array(
	/*	 PPTP	*/ 1723,
	/* OpenVPN 	*/ 1194, 443, 8888, 992
	);
	foreach($portumuz as $port)
	{
		if ($fp = fsockopen("tcp://".$kullanicip, $port, $errno, $errstr, 1))
		{
			$portac++;
			$portyakala .= "TCP: ".$port."<br>";
			fclose($fp);
		}
	}
	/* ############### TCP Bitti ############### */	
	
	/* ############### UDP Başladı ############### */
	
	$ports_udp = array(
	/* OpenVPN 	*/ 1194, 53, 80, 8888, 992
	);	
	foreach($ports_udp as $port_udp)
	{
		$fp = fsockopen("udp://".$kullanicip, $port_udp, $errno, $errstr);
		socket_set_timeout($fp, 1);	 
		fwrite($fp, "\x38\xa1\xd6\x91\xd6\xa2\x9f\xb3\x2b\x00\x00\x00\x00\x00");
		$alis = fread($fp, 8192);
		$alis_all .= $alis." (".(strlen($alis)).") <br>";
		$size = strlen($alis);
		if ( $size >= 25)
		{
			$portyakala .= "UDP: ".$port_udp."<br>";
			$portac++;
			$portac++;
		}
		fclose($fp);	
	}
	/* ############### UDP Bitti ############### */	

	/* ############### L2TP Başladı ############### */
	
	$fp = fsockopen("udp://".$kullanicip, 500, $errno, $errstr);
	socket_set_timeout($fp, 1);	
	fwrite($fp, "");
	$alis = fread($fp, 8192);
	$size2 = strlen($alis);
		if ( $size2 >= 100)
		{
			$portyakala .= "L2TP: 500 <br>";
			$portac++;
			$portac++;
		}
	fclose($fp);
		
	/* ############### L2TP Bitti ############### */
	
			return $portac;	
}
//VPN Test Başarılı

//////////////// FONKSIYON BİTTİ ////////////////

//////////////// BOT ////////////////
$query = new ts3admin($teamspeak['address'], $teamspeak['tcp']);

if($query->getElement('success', $query->connect()))
{
    $query->login($teamspeak['login'],$teamspeak['password']);
    $query->selectServer($teamspeak['udp']);
    $query->setName($bot['name']);
	$core = $query->getElement('data',$query->whoAmI());
    $query->clientMove($core['client_id'],$bot['default_channel']);
    $users = $query->getElement('data',$query->clientList('-groups -voice -away -times'));
    while(true) 
	{ 
    $users = $query->getElement('data',$query->clientList('-groups -voice -away -times'));
        
        foreach ($users as $client) 
		{
		$info = $query->getElement('data',$query->clientInfo($client['clid']));
		$user_groups = explode(',',$client['client_servergroups']);
			 
            if ( ($info['client_version'] != "ServerQuery") && (!(isInGroup($user_groups,$vpn_izin_verilengrup))) )
			{
				$clid = $client['clid'];
				if ( ((empty($user[$clid])) && (empty($kullanicip[$clid]))) || ($kullanicip[$clid] != $info['connection_client_ip']))
				{
					$user[$clid] = vpn($info['connection_client_ip']);
					$kullanicip[$clid] = $info['connection_client_ip'];
				}
				if (filter_var($info['connection_client_ip'], FILTER_VALIDATE_IP))
				{
					if ($user[$clid] >= 3)
					{	
						if ($message != "")
						{
							$query->sendMessage(1, $client['clid'], $message);
						}
						if ($poke != "")
						{
							$query->clientPoke($client['clid'], $poke);
						}

						$query->clientKick($client['clid'], "server", $atmayazisi." [".$user[$clid]."]");
						
						//unset ($user[$clid]);
						//giriş
						if ($log == 1)
						{
							$fp=fopen("log.txt", "a"); 
							flock($fp, 3); 
							fwrite($fp, "".date('d.m.Y H:i')."	");
							fwrite($fp, "VPN TESPİT EDİLDİ"." [".$user[$clid]."]"."	");
							fwrite($fp, "".$client['client_nickname']."	"); 
							fwrite($fp, "".$info['connection_client_ip']."	");
							fwrite($fp, "".$info['client_unique_identifier']."	");
							fwrite($fp, "\n");
							fclose($fp);
						}
						
						
					}
				}      
            }
        }

		
		
		
        sleep($bot['interval']);
    }
}
//////////////// BOT Bitti ////////////////
?>
